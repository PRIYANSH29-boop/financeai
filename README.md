...
| 21 | Regime-segmented backtest (calm/normal/stressed) | **Beta drift measured** — β0.75 pie 0.26 calm → 0.78 stress; cash sleeve halves stressed drawdown. In-sample, 2022 sole stress episode — directional only. **Corrected in #30:** its momentum comparison book breached the sector cap (Info Tech 44% vs 30%); selection now applies the ≤5-names/sector rule and both columns are published side by side. **Pie rows unchanged** | [report](figures/lab/regime_report.md) |
...
| 27 | The Signal Duel — frozen ML vs plain 12-1 momentum | **TRADE MOMENTUM — conclusive.** Identical construction, only the score varying, 47 OOS months: after-cost Sharpe **1.79 vs 1.41**. The ML's full-window Rank IC edge (0.0505 vs 0.0414) rests on 2 of 5 years and fails the pre-stated consistency clause. Conclusive because the A-1 contaminant *favours* the ML — it lost while carrying an advantage | [report](figures/lab/signal_duel.md) |
