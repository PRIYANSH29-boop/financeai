...
- **~50 multi-class large caps are MISSING from this universe (found in #24).** The share-count screen reads SEC's XBRL `frames` API, which returns only undimensioned facts; multi-class filers tag shares per share class, so Alphabet, Meta, Berkshire, Visa, Mastercard, Ford, Comcast and ~44 others had no share count and were dropped before the market-cap filter ran. Only 451 of the 503 S&P 500 names are present. The omission is non-random — it removes very large, mostly founder/family-controlled companies — so the cross-section this model was retrained on is not the one the §1 funnel describes. `universe.py` is fixed; a rebuilt panel is required for the numbers above to reflect it. See LIMITATIONS §10.
